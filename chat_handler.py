...
import time
import traceback
from ollama_handler import query_ollama, is_ollama_running
...
class ChatHandler:
    ...
    def _check_model_availability(self):
        ...
        if not is_ollama_running():
            logger.warning("Ollama is not running. Chat functionality may be limited.")
            return False
        
        try:
            # Simple query to check if model works
            test_prompt = "Hello! This is a test message. Please respond with 'OK'."
            test_response = query_ollama(test_prompt, self.model, max_retries=1)
            logger.info(f"Chat model {self.model} is available")
            return True
        except Exception as e:
            logger.warning(f"Chat model {self.model} may not be available: {str(e)}")
            return False
    ...
    def switch_to_data_mode(self, user_id):
        ...
        if success:
            return ("📊 *Data Cleaning Mode Activated*\n\n"
    ...
    def process_message(self, user_id, message_text):
        ...
        try:
            ...
            logger.info(f"Generated chat response in {end_time - start_time:.2f} seconds")
            ...
            return f"⚠️ I encountered an error while processing your message. Please try again later."
    ...
    def _prepare_context(self, user_id):
        ...
        if len(context) > MAX_CONTEXT_LENGTH:
            ...
            context = system_part + "...[older messages omitted]...\n" + recent_part
...
Your conversation history is saved to provide context for our discussions. Use /clear to reset it anytime."""
