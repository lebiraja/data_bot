...
from src.handlers.ollama_handler import query_ollama
...
def clean_and_summarize(file_path: str, output_dir: str = "outputs"):
    ...
    if file_size_mb > 100:
        ...
        raise ValueError(
            ...
            "Maximum size for processing is 100 MB."
        )
    ...
    try:
        ...
        if df.shape[0] > 1000000 or df.shape[1] > 100:
            ...
            raise ValueError(
                f"Dataset too large: {df.shape[0]} rows × {df.shape[1]} columns. "
                "Maximum is 1,000,000 rows and 100 columns."
            )
...
"""
        ...
        summary = (
            f"Could not generate complete statistics: {str(e)}\n"
            f"Basic info: {df.shape[0]} rows × {df.shape[1]} columns"
        )
...
Return only the steps you would perform and why."""
        ...
        ollama_response = (
            f"Could not get AI suggestions: {str(e)}\n"
    ...
    try:
                ...
                if missing_pct < 10:
                    ...
                    if rows_dropped > 0:
                        cleaning_steps.append(
                            f"Dropped {rows_dropped} rows with missing values in column '{column}'" # noqa: E501
                        )
                        logger.info(
                            f"Dropped {rows_dropped} rows with missing values in column '{column}'" # noqa: E501
                        )
                    ...
                    if pd.api.types.is_numeric_dtype(df[column]):
                        ...
                        cleaning_steps.append(
                            f"Filled {int(missing_pct * original_rows / 100)} missing values in numeric column '{column}' with median ({median_value})" # noqa: E501
                        )
                        logger.info(
                            f"Filled missing values in numeric column '{column}' with median" # noqa: E501
                        )
                    else:
                        mode_value = (
                            df[column].mode()[0]
                        ...
                        cleaning_steps.append(
                            f"Filled {int(missing_pct * original_rows / 100)} missing values in non-numeric column '{column}' with mode ({mode_value})" # noqa: E501
                        )
                        logger.info(
                            f"Filled missing values in non-numeric column '{column}' with mode" # noqa: E501
                        )
        ...
        cleaning_summary = (
            "\n".join(cleaning_steps)
            if cleaning_steps
            else "No automatic cleaning steps were necessary"
        )
        ...
        output_file = os.path.join(
            output_dir, f"cleaned_{timestamp}_{base_filename}"
        )
...
"""
        logger.info(
            f"Cleaning complete. Original: {original_rows} rows, Cleaned: {df.shape[0]} rows"
        )
