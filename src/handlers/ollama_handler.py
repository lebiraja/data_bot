...
def query_ollama_cli(prompt: str, model: str = "deepseek-r1:1.5b"):
    ...
    try:
        ...
        logger.debug(
            f"Ollama CLI request completed in {end_time - start_time:.2f} seconds"
        )
        ...
        raise Exception(
            "Ollama request timed out. The operation took too long to complete."
        )
...
def query_ollama(prompt: str, model: str = "deepseek-r1:1.5b", max_retries=2):
    ...
    if not is_ollama_running():
        ...
        raise Exception(
            "Ollama is not running or installed. Please start Ollama service."
        )

    # Truncate prompt if it's too long
    if len(prompt) > 15000:
        logger.warning(f"Truncating prompt from {len(prompt)} to 15000 characters")
        prompt = prompt[:14997] + "..."
        ...
        try:
            ...
            if USE_API:
                try:
                    ...
                    logger.warning(
                        f"API call failed: {str(api_error)}. Falling back to CLI."
                    )
            ...
            if retry_count <= max_retries:
                ...
                logger.warning(
                    f"Retrying Ollama query in {wait_time} seconds... (Attempt {retry_count}/{max_retries})"
                )
